...
(define (get-men mpref)
  (define (men-pref mpref stack)
    (if (null? mpref)
        (reverse stack)
        (men-pref (cdr mpref) (cons (caar mpref) stack))))
  (men-pref mpref '()))
...
(define (women-pref wpref acc)
  (if (null? wpref)
      (reverse acc)
      (women-pref (cdr wpref) (cons (caar wpref) acc))))  
...
(define (get-pref-list pref person)
 (cond
    ((null? pref) '())
    ((eq? (caar pref) person) (cdr (car pref)))
