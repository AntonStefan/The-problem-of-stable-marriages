...
(define (get-women wpref)
  (foldr (lambda (prefs women)
           (cons (car prefs) women))
         '()
         wpref))
...
(define (get-pref-list pref person)
    (apply append (map (lambda (prefs) (if (equal? (car prefs) person) (cdr prefs) '())) pref)))
...
(define (preferable? pref-list x y)
   (if (member y (member x pref-list)) #t
  #f))
...
(define (get-partner engagements person) 
     (define (partner-predicate e)
    (equal? (car e) person))
  
  (define partner (find-first partner-predicate engagements))
  (if partner
      (cdr partner)
      #f))
...
; rămâne neschimbată.
(define (change-first p L val) ;aici
  (cond ((null? L) '())
...
(define (update-engagements engagements p1 p2)
  (define old-partner (get-partner engagements p1))
(change-first (lambda (couple) (equal? couple (cons p1 old-partner))) engagements (cons p1 p2)))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
   (cond
    ...
    ((preferable? (get-pref-list pref2 (car p1-list)) p1 (get-partner
        (foldr (lambda (per acum) (cons (cons (cdr per) (car per)) acum)) '() engagements) (car p1-list))) #t)
     ((preferable? (get-pref-list pref2 (car p1-list)) p1 (get-partner
         engagements (car p1-list))) #t)
